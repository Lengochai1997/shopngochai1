...
@section('content')
                    ...
                    <div class="col-sm-12 col-md-5">
                        <div class="row">
                            <div class="col-sm-12 text-center">
                                ...
                                <h3 class="gold-count">Trong Hũ Vàng Có <span>{{ number_format($spin->total) }} {{ config('coin.type')[$spin->type] }}</span></h3>
                            </div>
                            <div class="col-sm-12 history-gold">
                                <h1 class="history-gold-title">Lịch sử nổ hũ vàng</h1>
                                    ...
                                    <table class="table">
                                        ...
                                        @if(count($spin->history_total) !== 0)
                                            @foreach($spin->history_total as $history)
                                                ...
                                                @endif
                                            <tr>
                                                <td>{{ $history->user->name ? $history->user->name : $history->user->username }}</td>
    ...
    <script type="text/javascript">
        function randomResult(elem) {
            ...
            callAjax('{!! asset('spin-coin/get-result') !!}/'+id, 'GET', {}, false).then(res => {
                if (res.ratio) {
            ...
            $({deg: 0}).animate({deg}, {
                ...
                step: function(now) {
                    ...
                    swal.fire({
                        ...
                        text: message,
                        icon: 'success',
                        footer: '<a href="{!! ($spin->type == 'kimcuong') ? asset('user/rut-kim-cuong') : asset('user/rut-quan-huy') !!}" class="btn btn-success" style="color: white;">Rút {!! ($spin->type == 'kimcuong') ? 'Kim cương' : 'Quân huy' !!}</a>'
                    });
